...
class ModifyPPM(torch.nn.Module):
    def __init__(self, in_dim, reduction_dim, bins):
        super(ModifyPPM, self).__init__()
        self.features = []
        for bin in bins:
            self.features.append(torch.nn.Sequential(
                torch.nn.AdaptiveAvgPool2d(bin),
                torch.nn.Conv2d(in_dim, reduction_dim, kernel_size=1),
                torch.nn.GELU(),
                torch.nn.Conv2d(reduction_dim, reduction_dim, kernel_size=3, bias=False, groups = reduction_dim),
                torch.nn.GELU()
            ))
        self.features = torch.nn.ModuleList(self.features)
        self.local_conv = torch.nn.Sequential(
            torch.nn.Conv2d(in_dim, in_dim, kernel_size=3, padding = 1, bias=False, groups = in_dim),
            torch.nn.GELU(),
        )
...
class Inception_block(torch.nn.Module):
    def __init__(self, in_dim, hidden_dim):
        super().__init__()
        self.down_project = torch.nn.Linear(in_dim,hidden_dim)
        self.act = torch.nn.GELU()
        self.mppm = ModifyPPM(hidden_dim, hidden_dim // 2 ,  [3, 5])
        self.up_project = torch.nn.Linear(hidden_dim, in_dim)
        self.down_conv = torch.nn.Sequential(torch.nn.Conv2d(hidden_dim*2, hidden_dim, 1),
                                       torch.nn.GELU())
...
class InceptionAapter(torch.nn.Module):
    def __init__(self, block: torch.nn.Module):
        super().__init__()
        self.in_dim = block.norm1.normalized_shape[0]
        self.adp = Inception_block(in_dim=self.in_dim, hidden_dim=self.in_dim//4)
        self.scaler = torch.nn.Parameter(torch.zeros(self.in_dim))
    ...
    def forward(self, x: torch.tensor) -> torch.tensor:
        ...
        feature = einops.rearrange(
            x[:, 1:, :], "b (h w) d -> b h w d", h=h, w=w
        )
        feature = self.adp(feature)
        feature = einops.rearrange(feature, "b h w d -> b (h w) d", h=h, w=w)
        feature = torch.cat([x[:, 0, :].unsqueeze(1), feature], dim=1)
        x = self.proj(x)

        return torch.einsum("bld,d->bld", feature, self.scaler) + x
...
class FreqLowRankAdapter(torch.nn.Module):
    """Low-rank adapter for linear layers."""
    ...
    def forward(self, x: torch.tensor) -> torch.tensor:
        x_scaled = x * self.in_scaler
        x_freq = dct.dct(x_scaled.permute(0, 2, 1)).permute(0, 2, 1).contiguous()
        x_freq = self.up(self.down(x_freq))
        x_freq = dct.idct(x_freq.permute(0, 2, 1)).permute(0, 2, 1).contiguous()
        x = self.linear(x_scaled)
        x_new = x + x_freq
        x = x_new * self.out_scaler
        return x
...
class FreqLowRankConvAdapter(torch.nn.Module):
    """Low-rank adapter for conv layers."""
...
class FreqLowRankConfigTimmViT:
    """Config to add low-rank adapters to a timm ViT."""
...
class AapterConfigTimmViT:
    ...
    def __init__(
        ...
        self.lora_rank = hidden_dim
        self.adapter_modules = "blocks"
        self.adapter_submodules = "blocks\.\d+"
...
class AdapterModule(torch.nn.Module):
    def __init__(self, in_feature, hidden_dim):
        ...
        self.proj_down = torch.nn.Linear(in_features=in_feature, out_features=hidden_dim)
        self.proj_up = torch.nn.Linear(in_features=hidden_dim, out_features=in_feature)
...
class Adapter(torch.nn.Module):
    ...
    def __init__(self, block: torch.nn.Module, hidden_dim=16):
        """Add a low rank adapter to a linear layer"""
        super().__init__()
        ...
        self.in_dim, self.out_dim = self.block.attn.proj.out_features, self.block.mlp.fc2.out_features
        self.adapter1 = AdapterModule(in_feature=self.in_dim, hidden_dim=hidden_dim)
        self.adapter2 = AdapterModule(in_feature=self.out_dim, hidden_dim=hidden_dim)
    ...
    def forward(self, x: torch.tensor) -> torch.tensor:
        x_input = self.block.drop_path1(self.block.ls1(self.block.attn(self.block.norm1(x))))
        x = x + self.adapter1(x_input)
        x_new = self.block.drop_path2(self.block.ls2(self.block.mlp(self.block.norm2(x))))
        x = x + self.adapter2(x_new)
        return x


def add_extra_weights_version2(
    model,
    config,
    trainable=True,
    only_scaler_trainable=False,
    ...
    for m_name, module in dict(model.named_modules()).items():
        if re.fullmatch(config.adapter_modules, m_name): 
            ...
            for submodule in module:
                adapter_instance = Adapter(submodule)
                new_blocks.append(adapter_instance)
            setattr(model, m_name, new_blocks)
...
def add_adapter(
    ...
    if type == "lora":
    ...
    elif type == "freq-low-rank":
        assert shared == False
        config = FreqLowRankConfigTimmViT(
            hidden_dim=hidden_dim,
            patch_embed=patch_embed_adapter,
            norm=norm_trainable,
        )
        model = add_extra_weights(
            ...
            config,
            FreqLowRankAdapter,
            FreqLowRankConvAdapter,
            adapter_trainable,
            only_scaler_trainable,
        )
    elif type == "adapter":
        assert shared == False
        config = AapterConfigTimmViT(
            hidden_dim=hidden_dim,
            patch_embed=patch_embed_adapter,
            norm=norm_trainable,
        )
        model = add_extra_weights_version2(
            model,
            config,
            adapter_trainable,
            only_scaler_trainable,
