...
class SAMSegWrapper(torch.nn.Module):
    """ViT with fully-convolutional head for dense predictions."""

    def __init__(self, backbone, feature_map_indices, num_classes, deepsup=True):
        super().__init__()
        self.backbone = backbone
        norm_cfg = dict(type="BN", eps=1e-6)
        act_cfg=dict(type='ReLU')
        self.deepsup=deepsup
        self.head = mmseg.models.decode_heads.SAMSegHead(
            img_size=(224, 224),
            patch_size=16,
            in_channels=[1024]*4,
            channels=256,
    ...
    def forward(self, x):
        ...
        output_map, aux_map = self.head(img=x, inputs=features)
