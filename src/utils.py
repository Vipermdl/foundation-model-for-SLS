...
def get_datamodule(
    ...
    train_transforms = torchgeo.transforms.AugmentationSequential(
    ...
    if config.data.few_shot_k is not None:
        ...
        elif "swed" in config.data.datamodule.lower():
            datamodule = datamodule(
                root=root,
                modality=config.data.modality,
                batch_size=config.optim.batch_size,
                ...
                val_transforms=additional_transforms # 
            )
        else:
            datamodule = datamodule(
                root=root,
    ...
    #     data = datamodule.train_dataset.__getitem__(idx)
    #     print(data['image'].unique())
        # if (data['mask'].unique()==torch.Tensor([0, 1, 255])).all():
        # if len(data['mask'].unique()) == 3:
...
def get_ckpt_path_from_wandb_run(
    ...
    run_path = glob.glob(
        os.path.join("logs/*", "wandb", f"*{getattr(config, run_id_key)}")
    )
    if not len(run_path):
